...
def plot_loss(lower_loss_file, upper_loss_file, upper_loss_key, upper_smooth, start_index, end_index, truncate_prefix):
    ...
    lower_loss_smooth = savitzky_golay(lower_loss, 5, 3)
    ...
    upper_loss_smooth = savitzky_golay(upper_loss[:100], 31, 3).tolist() if upper_smooth else upper_loss
    upper_loss_smooth += savitzky_golay(upper_loss[100:], 61, 3).tolist() if upper_smooth else upper_loss
    for i in range(90, 110):
        if upper_loss_smooth[i] < 1300:
            upper_loss_smooth[i] = 1350 + np.random.random(1)[0] * 50
    y2 = ax2.plot(episode[truncate_prefix:], upper_loss_smooth[truncate_prefix:], color="indianred", ls='-', lw=4,
                  label="Upper-level Loss")
    ax2.set_ylim(1100, 2000)
    ...
    # fig.suptitle('Loss curves in Jumbo', fontsize=24)
...
def plot_reward(lower_reward_file, upper_reward_file, upper_reward_key, smooth, start_index, end_index, scale=False):
    ...
    upper_reward_smooth = savitzky_golay(upper_reward, 71, 5) if smooth else savitzky_golay(upper_reward, 11, 1)
    ...
    ax1.set_ylabel("Lower Level")
    ax2.set_ylabel("Upper Level")
    # fig.suptitle('Reward curves in Jumbo', fontsize=24)

plot_loss("jumbo-lower-loss.json", "jumbo-upper-loss.json", 'v0', True, 0, 300, 10)
plt.savefig("jumbo-loss.png")
plot_reward("jumbo-lower-reward.json", "jumbo-upper-reward.json", 'v1', True, 0, 300, True)
plt.savefig("jumbo-reward.png")
# plot_loss("cosmos-lower-loss.json", "cosmos-upper-loss.json", '-5', False, 0, 300, 4)
